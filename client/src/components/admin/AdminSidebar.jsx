import { LayoutDashboardIcon, ListCollapseIcon, Sidebar,ListIcon ,PlusSquareIcon} from 'lucide-react'
       ...
       <p className='mt-2 text-base max-md:hidden'>
        ...
        <div className='w-full'>
         {adminNavlinks.map((link,index)=>(
             <NavLink key={index} to={link.path} end className={({isActive})=>`relative flex item-center max-md:justify-center gap-2 w-full py-2.5 min-md:pl-10 first:mt-6 text-gray-400 ${isActive && 'bg-primary/15 text-primary group'}`}>
                    {(isActive)=>(
                            <>
